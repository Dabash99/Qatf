...
faster-whisper + Codex, with a CLI and a FastAPI job server over one pipeline.

**Status: working prototype, not production.** Treat everything below
marked UNVERIFIED as unknown rather than working.

Inherits the global `~/.Codex/AGENTS.md` conventions. This file only covers what is
...
qatf-backend/      the Python pipeline, CLI and API — everything below lives here
  qatf/
    ...
    llm/             stage 3 providers — the only swappable part of the pipeline
      ...
      Codex.py      Anthropic Messages API, official SDK
...
Two SDK families, deliberately: the official `anthropic` SDK for Codex, and the
`openai` SDK for everything OpenAI-compatible. **Never route Codex through an
...
A rejected parameter is a 400, not a graceful degrade. Codex Opus 5 and the
...
picking a self-contained passage, hearing a hook — is unmeasured. Codex and GPT
...
- **Every provider except OpenRouter and Ollama, against its real endpoint.**
  OpenRouter has served a real request (Codex Opus 5, json_object tier).
  **Ollama has now served two** (`qwen3:14b`, json_object tier, real Arabic
  ...
  others.
- Arabic *selection* quality on any non-Codex provider (see Stage 3 providers).
...
- *Reading the .ass file.* It looks correct either way. This is the trap
  AGENTS.md already warned about, and it is real.
